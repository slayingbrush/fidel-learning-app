...
export function Navigation() {
  ...
  return (
    <nav aria-label="Main navigation" className="fixed bottom-0 left-0 right-0 z-50 bg-white border-t border-[var(--color-border)] safe-area-pb">
      <div className="max-w-lg mx-auto flex">
        {navItems.map(item => (
          <NavLink
            ...
            to={item.to}
            className={({ isActive }) =>
              `flex-1 flex flex-col items-center gap-1 py-2 px-1 tap-target transition-colors
              ...
              <>
                <div className="relative">
                  ...
                  {item.to === '/practice' && dueCount > 0 && (
                    <span className="absolute -top-1 -right-1 min-w-[16px] h-4 px-0.5 bg-amber-500 text-white text-[10px] font-bold rounded-full flex items-center justify-center leading-none badge-pulse">
